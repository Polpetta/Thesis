...
\paragraph*{Errori di interpretazione} È anche possibile che le persone
...
\end{figure}
PastBook basa i suoi servizi nel \glslink{cloud}{cloud} grazie a Amazon AWS
Services. Per realizzare il \textit{bot} è quindi stato deciso di pensarlo
utilizzando un'architettura \glslink{serverless}{serverless} con database non
relazionali (ovvero di tipo \glslink{nosql}{NoSQL}). Un'architettura di questo
...
programma nel \glslink{cloud}{cloud};
...
risorse;
  \item tutto l'ambiente è all'interno dello stesso \glslink{cloud}{cloud},
...
Il servizio \glslink{cloud}{cloud} utilizzato nello specifico si chiama Amazon
...
JavaScript, provvedendo quindi a generare un file compresso che li contenga
tutti e a caricarli nel servizio.
...
fini del progetto, a ogni \gls{url} è stata associato una chiamata a una
...
Amazon AWS APIGateway fornisce a Swagger. Si è proceduti quindi nella seguente
maniera:
\begin{enumerate}
...
utenti e gli album con gli indirizzi alle relative foto. Il trasferimento dei
dati è stato eseguito inviando direttamente oggetti di tipo
...
Le azioni eseguibili dal \textit{bot} sono la notifica all'utente che si
